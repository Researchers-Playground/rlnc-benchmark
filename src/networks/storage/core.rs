...
pub trait NodeStorage<'a, C: Committer<Scalar = Scalar>> {
    type Commitment;
...
pub struct InMemoryStorage<'a, C: Committer<Scalar = Scalar>> {
...
impl<'a, C: Committer<Scalar = Scalar>> InMemoryStorage<'a, C> {
    pub fn new() -> Self {
        Self {
            shreds: HashMap::new(),
            coded: HashMap::new(),
...
impl<'a, C: Committer<Scalar = Scalar>> NodeStorage<'a, C> for InMemoryStorage<'a, C> {
    ...
    fn store_decoded(&mut self, block: BlockId, shred: ShredId, decoder: NetworkDecoder<'a, C>) {
        self.decoded.insert((block, shred), decoder);
    }
