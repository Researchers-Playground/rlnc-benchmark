...
// Generic scalar trait that any field type must implement

use crate::utils::field::FieldScalar;

#[derive(Clone)]
pub struct Echelon<Scalar: FieldScalar> {
    coefficients: Vec<Vec<Scalar>>,
    echelon: Vec<Vec<Scalar>>,
    transform: Vec<Vec<Scalar>>,
}

impl<Scalar: FieldScalar> Echelon<Scalar> {
    ...
    pub fn add_row(&mut self, row: Vec<Scalar>) -> bool {
        ...
        let mut i = 0;
        let mut j: usize;
        let mut new_echelon_row: Vec<Scalar> = row.clone();
...
mod tests {
    ...
    #[test]
    fn test_add_row() {
        let mut echelon = Echelon::<Scalar>::new(3);
        assert_eq!(
            echelon.add_row(vec![
                Scalar::from(0u32),
                Scalar::from(0u32),
    ...
    fn test_compound_scalars() {
        let echelon = Echelon::<Scalar>::new(3);
        assert_eq!(
